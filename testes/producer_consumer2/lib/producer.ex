defmodule ProducerConsumer.Producer do
  ...
  defp send_consumer(cache_item) do
    Enum.each(1..4, fn _ ->
      send_consumer_process(cache_item)
    end)
  end

  defp send_consumer_process(cache_item) do
    # Spawn a new process for each consumer and send the message
    spawn_link(fn ->
      consumer_pid = self()  # Get the PID of the current process (not used directly)
